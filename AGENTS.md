...
- **Function programming**: prefer functional with pure functions, but do not over-engineer: DRY-methodlogy should not be followed 100%
- **Result & Option returns**: in languages, like JavaScript/TypeScript use Result to wrap return of operations that can throw an error and Option for operations that can return nullable (or undefined):
```ts
export type Some<T> = {
	readonly _tag: "Some";
...
```
- **Prefer pattern-matching where language supports or something like the following for other languages**:
```ts
...
...
const MATCH_VARIANTS = makeMatch({..})

const result = MATCH_VARIANTS[option]
```